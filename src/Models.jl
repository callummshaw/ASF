...
function ASF_M3_full(out,u,p,t)
    ...
    for i in 1:pop_data.pop #looping through populations!
        ...
        deff_s[i] = d_eff
        Lambda = @. p.λ[si:ei] + p.la[si:ei] * cos((t + p.lo[si:ei]) * 2*pi/year) #decay
        p_mag = @. p.k[si:ei]*exp(-p.bw[si:ei]*cos.(pi*(t+p.bo[si:ei])/365)^2) #birth pulse value at time t
        Deaths = @. p.μ_p[si:ei]*(0.75 + ((1-0.75))*sqrt(Np/K))*p.g[si:ei] #rate
        ...
        out[11*s0+3 :11:11*ei] .= d_eff.*p.β[si:ei,si:ei].*S./NV * (I.+p.ω[si:ei].*C) 

...
struct Model_Data
    ...
    function Model_Data(Path; verbose = false, fitting_rewire = 0)
        ...
        S0 = build_populations(sim, pops, network, counts) #initial populations
...
function beta_construction(sim, pops, counts, network)
    ...
    if Mn == 3 
        for i in 1:n_pops #iterating through
            ...
            if !sim.Fitted
                ...
                if sim.Network == "s"
                    path = "../Inputs/Fitted_Params/Posteriors/Tau-Hetrogeneous/Scale_Free/"
                elseif sim.Network == "w"
                    path = "../Inputs/Fitted_Params/Posteriors/Tau-Hetrogeneous/Small_Worlds/"
                else 
                    path = "../Inputs/Fitted_Params/Posteriors/Tau-Hetrogeneous/Random/"
                end
    ...
    elseif Mn == 2 #tau Homogeneous model
        ...
        if !sim.Fitted
            ...
            path = "../Inputs/Fitted_Params/Posteriors/Tau-Homogeneous/contact_out.csv"
            rand_values = rand(1:10000,1)
        ...
        if !sim.Fitted
            ...
            path = "../Inputs/Fitted_Params/Posteriors/ODE/contact_out.csv"
            rand_values = rand(1:10000,1)
